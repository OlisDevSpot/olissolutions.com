import type { InsertTrade } from '@olis/db/schema/one-stop-sales'
...
import { addons, solutions, trades } from '@olis/db/schema/one-stop-sales'
...
export async function findAllSolutionsByTradeAccessor(accessor: string) {
  const rows = await db
    .select({
      trade: trades,
      solution: solutions,
    ...
    .leftJoin(solutions, eq(solutions.tradeId, trades.id))
