...
import * as repository from '@olis/server/routers/platform/customers/repository'
...
export const customersRouter = createTRPCRouter({
  ...
  createOne: protectedProcedure.input(insertCustomerSchema.strict()).mutation(async ({ input }) => {
    const customer = await repository.createOne(input)
    return customer
  }),
  updateOne: protectedProcedure.input(z.object({ id: z.string(), ...insertCustomerSchema.strict().partial().shape })).mutation(async ({ input }) => {
