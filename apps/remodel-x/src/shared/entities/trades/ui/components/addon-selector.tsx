import { useGetTradeAddons } from "@olis/data-client/fetchers/platform/trades/queries/get-trade-addons";
...
export function AddonSelector({ currentTradeId }: Props) {
  const { selectedAddons } = useProjectScopesStore();
  const addonsOfTrade = useGetTradeAddons(currentTradeId!, { enabled: currentTradeId !== null });

  if (addonsOfTrade.isLoading) {
    return (
      <LoadingState
  ...
  const addons = addonsOfTrade.data?.addons || [];

  if (!addonsOfTrade.data || addons.length === 0) {
    return (
      <EmptyState 
        title="No addons" 
